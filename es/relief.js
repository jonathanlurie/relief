import { PerspectiveCamera, Scene, AmbientLight, PointLight, WebGLRenderer, Raycaster, Vector2, TorusKnotBufferGeometry, MeshPhongMaterial, Mesh, Vector3, Quaternion, EventDispatcher, MOUSE, Spherical, Object3D, PlaneBufferGeometry, MeshBasicMaterial, DoubleSide, Matrix4, CylinderBufferGeometry } from 'three';
...
class ThreeContext extends EventManager {
  ...
  constructor(divObj = null) {
    ...
    // init camera
    this._camera = new PerspectiveCamera(27, divObj.clientWidth / divObj.clientHeight, 1, 50000);
    ...
    let bulbLight = new PointLight( 0xffffff, 1, 0, 1 );
    bulbLight.position.set( -1000, 500, -1000 );
    this._scene.add( bulbLight );


    let bulbLight2 = new PointLight( 0xffffff, 0.5, 0, 1 );
...
class ReliefBuilder {
  ...
  createFromRaster(data, width, height) {
    ...
    let geometry = new PlaneBufferGeometry(width-1, height-1, width-1, height-1);
